...
class DataPreprocessor:
    ...
    def scale_features_sets(self, X_train, X_test):
        ...
        scaler = MinMaxScaler()
        feature_columns = ["temperature", "humidity", "wind_speed", "general_diffuse_flows", "diffuse_flows"]
    ...
    def split_data(self, test_size=0.2, random_state=42):
        ...
        X = self.data.drop(columns=["zone_1_power_consumption", "zone_2_power_consumption", "zone_3_power_consumption"])

        y = self.data[["zone_1_power_consumption", "zone_2_power_consumption", "zone_3_power_consumption"]]

        X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=test_size, random_state=random_state)
        self.logger.info(
            f"Data split into training and testing sets with test_size={test_size} and random_state={random_state}."
        )
